...
# LIFO Policy. It allows to propagate through a URLs tree.
class LIFO_Policy:
    def __init__(self, seedURLs):
        self.urls = seedURLs.copy()
    ...
    def updateURLs(self, c, retrievedURLs, retrievedURLsWD, iteration):
        tmpRetrieved = list(retrievedURLs.copy())
        tmpRetrieved.sort(key=lambda url: url[len(url)-url[::-1].index('/'):])
        self.urls.extend(tmpRetrieved)

# LIFO Cycle Policy. It allows to propagate through a URLs tree avoids cycles.
class LIFO_Cycle_Policy:
    def __init__(self, seedURLs):
        self.urls = seedURLs.copy()
        self.fetched = set()
    
    def clear(self, seedURLs):
        self.urls = seedURLs.copy()
        self.fetched = set()
    ...
    def getURL(self, c, iteration):
        while True:
            if len(self.urls) == 0:
                ...
                elem = self.urls.pop()
                if elem in self.fetched:
                    continue
                self.fetched.add(elem)
                return elem
...
# LIFO Authority Policy. It allows to propagate through a URLs tree avoids cycles.
class LIFO_Authority_Policy:
    ...
    def clear(self, authority, explore = True):
        self.urlsAuth = np.array(list(authority.keys()))
        values = np.array(list(authority.values()))
        self.probabilities = np.array(list(authority.values()))
    ...
    def getURL(self, c, iteration):
        if self.explore:
            ...
            return np.random.choice(self.urlsAuth, 1, list(self.probabilities))[0]
...
# FIFO Policy. It allows to propagate through a URLs tree.
class FIFO_Policy:
    def __init__(self, seedURLs):
        self.urls = seedURLs.copy()
...
def main():
    ...
    for iteration in range(c.iterations):
        ...
        if (c.toFetch == None):
            ...
            c.generatePolicy.clear(c.authority,  explore = False)
            continue
...
def getNormalisedURLs(retrievedURLs):
    return [item.lower() for item in retrievedURLs]
    
#-------------------------------------------------------------------------  
# Remove duplicates (duplicates) 
def removeDuplicates(c, retrievedURLs):
    setRetrievedURLs = set(retrievedURLs)

    notDupplicated = [item for item in setRetrievedURLs if item not in c.URLs]
    return notDupplicated

#-------------------------------------------------------------------------  
# Filter out some URLs (TODO)
def getFilteredURLs(c, retrievedURLs):
    toLeft = list(set([url for url in retrievedURLs if url.startswith(c.rootPage)]))
    while c.toFetch in toLeft:
